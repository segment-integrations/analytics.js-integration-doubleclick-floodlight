...
var Floodlight = module.exports = integration('DoubleClick Floodlight')
  .option('source', '')
  .tag('standard', '<img src="http://ad.doubleclick.net/activity;src={{ src }};type={{ type }};cat={{ cat }};ord={{ ord }}?">')
  .tag('custom', '<img src="http://ad.doubleclick.net/activity;src={{ src }};type={{ type }};cat={{ cat }};ord={{ ord }};{{ customVariables }}?">')
  .mapping('events');
...
Floodlight.prototype.track = function(track) {
  ...
  var tags = foldl(function(conversions, tag) {
    ...
    var tagParams = {
      ...
      cat: tag.cat,
      ord: cacheBuster,
      isCustom: tag.customVariable.length, // For conditional tag loading later
      customVariables: qs.stringify(matchedVariables).replace('&', ';')
    };
  ...
  each(function(tagParams) {
    var tagType = tagParams.isCustom ? 'custom' : 'standard';
    return self.load(tagType, tagParams);
  }, tags);
};
